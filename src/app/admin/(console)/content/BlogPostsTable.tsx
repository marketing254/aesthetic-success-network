...
const STATUS_COLOR: Record<BlogPostRow["status"], { bg: string; fg: string }> = {
...
const STATUSES: BlogPostRow["status"][] = ["draft", "published", "archived"];
...
export default function BlogPostsTable({ initialRows }: { initialRows: BlogPostRow[] }) {
  const [rows, setRows] = useState<BlogPostRow[]>(initialRows);
  ...
  const setStatus = async (row: BlogPostRow, status: BlogPostRow["status"]) => {
  ...
  return (
    <Stack spacing={3}>
      ...
      <Grid container spacing={2}>
        {[
          { label: "Total posts", value: counts.total, accent: false },
        ...
        {filtered.length === 0 ? (
          <Box sx={{ p: 5, textAlign: "center" }}>
            <Typography variant="h5" sx={{ mb: 1, color: "text.primary" }}>
              {rows.length === 0 ? "No posts yet" : "No matches"}
            </Typography>
                  ...
                  return (
                    <Box component="tr" key={row.id}>
                      ...
                      <Box component="td">
                        <Select
                          value={row.status}
                          onChange={(e) => setStatus(row, e.target.value as BlogPostRow["status"])}
                          size="small"
                          sx={{
                            fontSize: "0.75rem",
                            height: 30,
